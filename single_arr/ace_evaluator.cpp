...
void ACEEvaluator::init(ACEAbstractBasisSet *basis_set) {
    ...
    //B_all.init(basis_set->total_basis_size_rank1[SPECIES_TYPE 0] + basis_set->total_basis_size[SPECIES_TYPE 0], "B_all");
    B_all.init(681, "B_all");
...
//        ndensity = func->ndensity;
        ...
        //B1_arr(func->mus[0],func_rank1_ind) += func->ctildes[0]*A_cur; //ctildes[0] because we should only sum over 1 density to get invariant
        //! add to all_invariant_array
...
//        ndensity = func->ndensity;
		...
		//!
            ...
            //B_arr(func->mus[rank],func->ns[rank],func->ls[rank]) += B.real_part_product(func->ctildes[ms_ind *ndensity +0]);//+0 at end for density_index=0
            DENSITY_TYPE P0 = 0;
            B_all(total_basis_size_rank1+func_ind) += B(func_ind).real_part_product(func->ctildes[ms_ind *ndensity +P0]);

            for (DENSITY_TYPE p = 0; p < ndensity; ++p) {
                //real-part only multiplication
                rhos(p) += B(func_ind).real_part_product(func->ctildes[ms_ind * ndensity + p]);
#ifdef EXTRA_C_PROJECTIONS
                //aggregate C-projections separately
                basis_projections(func_ind, p)+=B(func_ind).real_part_product(func->ctildes[ms_ind * ndensity + p]);
#endif
...
/* ---------------------------------------------------------------------- 
double ACECTildeEvaluator::memory_usage()
{
