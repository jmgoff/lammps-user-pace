...
void ACECTildeEvaluator::init(ACECTildeBasisSet *basis_set) {
    ...
    int tr1,tr, muind = 0;
    //for (muind ; muind < basis_set->nelements;muind ++){
    tr1 = basis_set->total_basis_size_rank1[muind];
    tr = basis_set->total_basis_size[muind];//change to mu_i ?
    //}
    B_all.init(tr1+tr);
...
ACECTildeEvaluator::compute_atom(int i, DOUBLE_TYPE **x, const SPECIES_TYPE *type, const int jnum, const int *jlist) {
    ...
    int tr1,tr, muind, offst = 0;
    //for (muind ; muind < basis_set->nelements;muind ++){
    //    element_offsets(muind) = offst;
    tr1 = basis_set->total_basis_size_rank1[mu_i];
    tr = basis_set->total_basis_size[mu_i];
    //offst += basis_set->total_basis_size_rank1[muind] + basis_set->total_basis_size[muind];
    //}

    neighbours_dB.resize(tr1+tr,jnum,3);
...
//        ndensity = func->ndensity;
        ...
        printf("     coeff[0] = %f\n", func->ctildes[0]);
#endif
        //printf("function index, elem offset: %d %d\n", func_rank1_ind); // , element_offsets(mu_i));
        //printf("function index: %d \n", func_rank1_ind); // , element_offsets(mu_i));
        //B_all(func_rank1_ind + element_offsets(mu_i)) += func->ctildes[0]*A_cur; //chemical offset?
        B_all(func_rank1_ind) += func->ctildes[0]*A_cur; //chemical offset?
...
//        ndensity = func->ndensity;
        ...
        for (ms_ind = 0; ms_ind < func->num_ms_combs; ++ms_ind, ++func_ms_ind) {
            ...
            DENSITY_TYPE dp = 0; //linear density contribution only
            B_all(func_ind + total_basis_size_rank1) += B.real_part_product(func->ctildes[ms_ind *ndensity + dp]); //works for ndensity>1 ?
            for (DENSITY_TYPE p = 0; p < ndensity; ++p) {
    ...
    // rank = 1
    int dp =0; //first density index for ctilde breakout
