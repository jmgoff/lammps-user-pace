...
def run_lammps(dgradflag):
    ...
    lmp.command('mass  1 15.999')
    lmp.command('mass  2 238.0289')

    lmp.command("displace_atoms 	all random 0.001 0.001 0.001 123456")

    # potential settings

    lmp.command(f"pair_style 	zero 12.5")
    ...
    if dgradflag:
	    lmp.command(f"compute 	pace all pace UO2_potential.yace 1 1")
    else:
	    lmp.command(f"compute 	pace all pace UO2_potential.yace 0 0")
